...
function calculateQarzolhasanehLoan(
  principal: number,
  annualRate: number,
  months: number,
): LegacyLoanResult {
  ...
  switch (loanType) {
    ...
    case 'qarzolhasaneh':
      ...
      if (calculationType === 'installment') {
        ...
        throw new Error(
          `محاسبه ${calculationType} برای وام قرض‌الحسنه در حال حاضر پشتیبانی نمی‌شود.`,
        );
      }
    ...
    case 'stepped':
      ...
      if (calculationType === 'installment') {
        ...
        throw new Error(
          `محاسبه ${calculationType} برای وام اقساط پلکانی در حال حاضر پشتیبانی نمی‌شود.`,
        );
      }
