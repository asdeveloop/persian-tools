...
function calculatePaymentSchedule(
  ...
  const totalPeriods = paymentFrequency === 'monthly' ? durationMonths : 
                      paymentFrequency === 'quarterly' ? Math.floor(durationMonths / 3) : 
  ...
  for (let period = 1; period <= totalPeriods; period++) {
    ...
    const _principal = installment - interest;
...
function calculateDueDate(startDate: Date, period: number, frequency: 'monthly' | 'quarterly' | 'yearly'): string {
...
function calculateLatePenalty(monthlyInstallment: number, lateMonths: number, penaltyRate: number): number {
  if (!lateMonths || !penaltyRate) return 0;
...
function calculateFees(amount: number, feeFlat?: number, feePercent?: number): number {
  let totalFees = 0;
  if (feeFlat) totalFees += feeFlat;
  if (feePercent) totalFees += amount * (feePercent / 100);
  return totalFees;
}
...
export function calculateLoan(input: LoanInput): LoanOutput {
  ...
  // Input validation
  if (amount <= 0) throw new Error('مبلغ وام باید بزرگ‌تر از صفر باشد.');
  if (durationMonths <= 0) throw new Error('مدت وام باید بزرگ‌تر از صفر باشد.');
  if (annualInterestRate < 0) throw new Error('نرخ سود سالانه نامعتبر است.');
