...
- TypeScript strict + تست برای قابلیت‌های اصلی (پوشش ۱۰۰٪)
...
- 🎯 CSP به‌روزشده: همه سرور اسکریپت‌های JSON-LD حالا با `middleware.ts` header `Content-Security-Policy` و `x-csp-nonce` ارسال می‌شوند تا `next/script` فقط با nonce اجرا شود.
